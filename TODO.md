...
- **DÍVIDA DIFERIDA (registada, não abandonada):** a reconciliação da **suite legada completa** (os ~11 specs mutáveis, ex.: `transactions-ledger` × `csv-import`) fica para o C4. Motivo factual: o mecanismo correcto para matar a contradição é **reset de banco por run**, que é precisamente o que o C4 constrói (Supabase efémero semeado). Reconciliar 11 specs contra o Cloud partilhado agora seria trabalho arquiteturalmente ao contrário. Esses specs continuam como regressão só-local até o banco efémero existir.
