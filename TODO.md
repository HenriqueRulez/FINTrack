...
- [ ] **U-01 — `/tax-calculator` desligado do ledger (mock) sem indicação visual** _(prioridade: média — dívida do AUDIT, fora do escopo do ciclo de 2026-08)_
  - **Estado (2026-08-06):** Holdings e Performance foram **religados ao ledger real** na Etapa 3 do AUDIT (`973bcc0`) — deixaram de ser mock. Sobra **só o `/tax-calculator`**: `TaxCalculatorPage.tsx` é 100% `mock-data.ts` (`SAMPLE_EVENTS_2026`/`EMPTY_EVENTS`/`TAX_SETTINGS`), zero chamadas de rede, com TODO explícito "replace mock events/settings with real data". Por default (`useSampleData=false`) mostra zeros honestos e o sample está atrás de um toggle rotulado — risco de "valor falso silencioso" é baixo, por isso ficou **fora de escopo** do A-03 por decisão do dono (2026-08-06).
  - **Como aplicar:** feature nova via pipeline completa (PO → … → Security) — derivar ganhos de capital + dividendos das `transactions` reais (a rota `GET /api/portfolio/history` já dá closes; o ledger dá as vendas/dividendos). Até lá, opcional: badge discreto "demo data" no `TaxPageHead` quando o sample estiver ligado.
...
- [ ] **G-05 — Suite e2e contém spec de uma página que já não existe**
  ...
  - **Actualização 2026-08-06 (E2E desbloqueado — suite corrida pela 1ª vez autenticada):** com o user de teste dedicado (`E2E_EMAIL`/`E2E_PASSPHRASE`), a suite completa passou a executar e revelou defeitos de isolamento/desactualização a tratar numa passagem de manutenção dedicada:
    - **Logout envenena a sessão partilhada:** o teste `CA-06` de `dashboard-visual-redesign.spec.ts` clica no logout, que faz `supabase.auth.signOut()` **global** (`logout-button.tsx:16`), revogando a sessão do user. Todos os testes a seguir (ordem alfabética) recebem redirect para `/passphrase` / `401`. Fix: `signOut({ scope: 'local' })` na app (mudança de comportamento, decidir com o dono) OU isolar o teste (user/contexto dedicado, correr por último).
    - **`transactions-ledger.spec` desactualizado:** espera 13 transações mock semeadas pela migration `0009` (removidas no F-04) — CA-02/CA-03/CA-04 falham (esperam "13 transações", "Cash 2 linhas", etc.). Reescrever contra fixtures próprias (como holdings/performance) ou dados reais.
    - **Contaminação de estado entre specs:** holdings/performance/transactions partilham um só ledger do user de teste → resultados flaky (o mesmo teste passou numa corrida e falhou noutra). Fix: cada spec usar tickers únicos + cleanup transaccional, ou serializar com limpeza garantida.
    - **Drift spec-vs-UI (holdings/performance):** empty-state ("Ainda não há posições"), error-state ("Não foi possível carregar"), formato "| EUR" na company cell, opacity 0.55 da linha fechada, selector `.overflow-x-auto`, alerta em empty-state de performance. Realinhar asserts com a UI actual.
    - **Já provado a funcionar:** infra de auth E2E (`auth.setup.ts` via `@supabase/ssr` + `@next/env` no `playwright.config.ts`) e as specs centrais de holdings com dados reais (KPI strip, Realized P/L, tabela, sort) numa corrida isolada limpa.
