...
| TD-5 | Preencher auditoria de tokens (D2) no fim da próxima feature | Baixa | Resolvido | Tabela D2 preenchida com 2 features reais (TD-7/FIN-9, BUG-6/FIN-14); QA = fase mais cara (50-56%); alvo = banco de teste efémero (TD-1/G-05) | Mínimo — preencher tabela no TODO.md | FIN-6 |
...
| TD-7 | Atualizar @supabase/ssr 0.6.1→0.12.4 para fechar B-15/B-18 (casts de write) | Média | Resolvido | 4 casts removidos; @supabase/ssr@0.12.4 corrige a ordem de genéricos → writes tipam sem `as any`. QA 43/43 verde, Security aprovado | Médio — bump de 6 minors mexe na API de cookies de auth; exige QA/security | FIN-9 |
