...
| B-15 | `src/lib/portfolio/prices.ts:106` (write) | Higiene de tipos: `(supabase as any)` no `upsert` de `price_cache`. NÃO é bypass de segurança — RLS de `price_cache` activo + GRANT limitado a `authenticated`. **Parcialmente resolvido em FIN-7/TD-6 (2026-08-08):** o cast de leitura (`prices.ts:51-53`) foi removido (anotação de tipo). O cast de escrita PERMANECE porque a causa raiz NÃO é o `database.ts` (já regenerado com `__InternalSupabase`) mas a incompatibilidade `@supabase/ssr@0.6.1` × `@supabase/supabase-js@2.112.1`: o `ssr` tipa o client com a ordem de genéricos antiga `<Database, SchemaName, Schema>`, colidindo com a nova assinatura de `SupabaseClient` (`<Database, SchemaNameOrClientOptions, SchemaName, Schema, ClientOptions>`) → o `Schema` colapsa para `never`, logo o payload de write não é atribuível. Writes não podem usar o truque da anotação (`never` não é atribuível a partir de um objecto). Resolve actualizando `@supabase/ssr` para versão compatível com a nova assinatura | M-03 AUDIT (cache persistente de preços) | 2026-08-06 |
...
| B-18 | `src/app/api/transactions/import/route.ts:201` | Higiene de tipos: `(supabase as any)` no batch `insert` do ledger. NÃO é bypass de segurança — `user_id` vem da sessão (`user.id`), RLS de `transactions` activo, insert parametrizado via postgrest (sem concatenação SQL). **Reavaliado em FIN-7/TD-6 (2026-08-08):** a causa raiz NÃO era o `database.ts` à mão (já regenerado com `__InternalSupabase`) mas a incompatibilidade `@supabase/ssr@0.6.1` × `@supabase/supabase-js@2.112.1` que colapsa o `Schema` para `never` (mesma raiz do B-15 write). O cast permanece; regenerar `database.ts` não resolve — resolve actualizar o `@supabase/ssr`. Nota: os mesmos casts de write existem, não rastreados por finding próprio, em `api/transactions/route.ts:160` (insert) e `api/transactions/[id]/route.ts:186` (update) | CSV Import (Trading212) | 2026-08-06 |
