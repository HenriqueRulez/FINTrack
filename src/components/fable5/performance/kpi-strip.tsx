...
// Cópia adaptada de src/components/performance/KPIStrip.tsx — 5 KPIs com
// micro-visualizações (Gauge, SplitBar, TickRow). Design do raiz mantido;
// os valores vêm calculados de dados REAIS do ledger.
...
export type TickState = "active" | "gain" | "loss" | "off";

function TickRow({ distribution }: { distribution: TickState[] }) {
  function tickClass(state: TickState): string {
    switch (state) {
      case "active": return "bg-primary";
      case "gain": return "bg-[var(--gain)]";
      case "loss": return "bg-[var(--loss)]";
  ...
  return (
    <div className="bg-card border border-border/50 rounded-lg overflow-hidden">
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5">
        {/* KPI 1 — Win Rate */}
        <div className="p-5 flex flex-col gap-3 min-w-0 border-r border-border/50">
        ...
        {/* KPI 2 — Profit Split */}
        <div className="p-5 flex flex-col gap-3 min-w-0 md:border-r md:border-border/50">
          <div className="flex items-center justify-between gap-2">
        ...
        <div className="p-5 flex flex-col gap-3 min-w-0 border-t border-r border-border/50 md:border-t-0 md:border-r-0 xl:border-r xl:border-border/50">
          <div className="flex items-center justify-between gap-2">
